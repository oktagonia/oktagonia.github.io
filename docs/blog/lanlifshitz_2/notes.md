...
We can answer this question by looking at the Cartesian form of this
...
if, for all $s$, $f(ax_1,\dots,ax_n) = a^kf(x_1,\dots,x_n)$ for some 
...
and the sum that determines $p$ is given by $\sum_{i=0}^n \binom{n}{i}x^ny^{n-i}.$
This makes things much clearer. We can see that $p$ is homogeneous because
$(ax^n)(ay)^{n-i} = a^na^{n-i}x^ny^{n-i} = a^nx^ny^{n-i}$ which allows us to factor
...
We may now move on to the conservation law associated with the 
...
Previously, the invariances with which we had to deal with were on the simpler
...
We can see that $BD = OD\sin\theta.$ Since $OD$ is just the position of the particle
...
As we discussed in the previous section, $\partial L/\partial \vec{v} = \vec{p}$
...
An interesting property of the center of mass is that if we can balance
...
second system, we have no choice but to compare it to the Lagrangian of
...
We have successfully found the condition for similarity between two mechanical systems,
...
orbiting the sum, the potential energy is $U = -GmM/r$ where $r$ is the distance
...
$q_i \to q_i + \varepsilon Q_i(q_1,\dots,q_n)$ is a symmetry.

<details>
<summary>See hint</summary>
Let $\delta q = \varepsilon Q_i(q_1,\dots,q_n)$ and 
$\delta \dot{q} = \d \delta q/\d t.$ 
</details>
...
$$
\sum_{i=1}^n\left( \frac{\partial L}{\partial q}\delta q + 
...
$$
\sum_{i=1}^n\frac{\d}{\d t}\left( \frac{\partial L}{\partial \dot{q}}\delta q\right) 
= \varepsilon\frac{\d}{\d t}\sum_{i=1}^n \frac{\partial L}{\partial \dot{q}}Q_i(q_1,\dots,q_n)
= 0.
$$
